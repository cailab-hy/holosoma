...
class CALQLAgent(BaseAlgo):
    config: CALQLConfig
    env: CALQLEnv  # type: ignore[assignment]
    actor: Actor
    qnet: DoubleQCritic
    ...
    def __init__(
        ...
        wrapped_env = CALQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
    ...
    def _validate_calql_offline_dataset(self) -> None:
        if not self.config.calql_use_mc_return:
            logger.warning("CAL-QL mc_return calibration is disabled; agent will behave like CQL/O2O.")
            return
        ...
        with h5py.File(self._offline_dataset_path, "r") as h5_file:
            if "mc_return" not in h5_file:
                message = (
                    "CAL-QL requires offline dataset key 'mc_return'. "
            ...
            if self.config.calql_validate_complete_episodes:
                if "episode_data_complete" not in h5_file:
                    raise KeyError(
                        "CAL-QL complete-episode validation requires key 'episode_data_complete'. "
    ...
    def setup(self) -> None:
        ...
        logger.info(f"CAL-QL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized CAL-QL model parameters across {self.gpu_world_size} GPUs")
    ...
    def online_learn(self, max_steps: int | None = None) -> None:
        ...
        while self.global_step < target_step:
            ...
            with self.logging_helper.record_learn_time():
                for _ in range(1): #(args.num_updates):
