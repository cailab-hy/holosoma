...
from holosoma.agents.cal_ql.cal_ql import Actor, CNNActor, DoubleQCritic
...
class OnlineReplayBuffer:
    def __init__(
        ...
        *,
        ...
        if capacity <= 0:
            raise ValueError(f"online_buffer_size must be positive, got {capacity}")
        ...
        self.observations = torch.zeros((capacity, observation_dim), device=device, dtype=torch.float32)
        self.actions = torch.zeros((capacity, action_dim), device=device, dtype=torch.float32)
        self.critic_observations = torch.zeros((capacity, critic_observation_dim), device=device, dtype=torch.float32)
        self.next_observations = torch.zeros((capacity, observation_dim), device=device, dtype=torch.float32)
        self.next_critic_observations = torch.zeros((capacity, critic_observation_dim), device=device, dtype=torch.float32)
    ...
    def add(self, transition: TensorDict) -> None:
        batch_size = int(transition.batch_size[0])
        if batch_size <= 0:
            return

        if batch_size >= self.capacity:
            transition = transition[-self.capacity :]
            batch_size = self.capacity

        indices = (torch.arange(batch_size, device=self.device) + self.ptr) % self.capacity
        self.observations[indices] = transition["observations"].to(device=self.device, dtype=torch.float32)
        self.actions[indices] = transition["actions"].to(device=self.device, dtype=torch.float32)
        self.critic_observations[indices] = transition["critic_observations"].to(device=self.device, dtype=torch.float32)
        self.next_observations[indices] = transition["next"]["observations"].to(device=self.device, dtype=torch.float32)
        self.next_critic_observations[indices] = transition["next"]["critic_observations"].to(
            device=self.device,
            dtype=torch.float32,
        )
        self.rewards[indices] = transition["next"]["rewards"].to(device=self.device, dtype=torch.float32).reshape(-1)
        self.dones[indices] = transition["next"]["dones"].to(device=self.device, dtype=torch.long).reshape(-1)
        self.truncations[indices] = transition["next"]["truncations"].to(device=self.device, dtype=torch.long).reshape(-1)
        self.effective_n_steps[indices] = transition["next"]["effective_n_steps"].to(
            device=self.device,
            dtype=torch.long,
        ).reshape(-1)
    ...
    def sample(self, batch_size: int) -> dict[str, Any]:
        if self.size <= 0:
            raise RuntimeError("Online replay buffer is empty. Collect online steps before online_learn().")
        if batch_size <= 0:
        ...
        return {
            ...
            "next": {
                ...
                "truncations": self.truncations.index_select(0, indices),
                "effective_n_steps": self.effective_n_steps.index_select(0, indices),
            },
        }
...
class OS_CALQLAgent(BaseAlgo):
    config: OS_CALQLConfig
    env: OS_CALQLEnv  # type: ignore[assignment]
    actor: Actor
    qnet: DoubleQCritic
    ...
    def __init__(
        ...
        wrapped_env = OS_CALQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
    ...
    def setup(self) -> None:
        ...
        logger.info(f"OS-CAL-QL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        ...
        self.online_replay_buffer = OnlineReplayBuffer(
            capacity=args.online_buffer_size,
            observation_dim=self.actor_obs_dim,
            critic_observation_dim=self.critic_obs_dim,
            action_dim=n_act,
            device=self.device,
        )
        ...
        logger.info(
            ...
            f"online_total_steps={args.online_total_steps}, "
            f"online_warmup_steps={args.online_warmup_steps}, "
            f"mixing_ratio_schedule={args.mixing_ratio_schedule}"
        )
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized OS-CAL-QL model parameters across {self.gpu_world_size} GPUs")
    ...
    def _update_q(
            ...
            with self._maybe_amp():
                ...
                curr_boundrate = torch.zeros((), device=self.device, dtype=bellman_loss.dtype)
                next_boundrate = torch.zeros((), device=self.device, dtype=bellman_loss.dtype)
                ...
                if self._cql_weight > 0.0:
                    ...
                    lse1 = torch.logsumexp(cat_q1 / self._temperature, dim=1) * self._temperature
                    lse2 = torch.logsumexp(cat_q2 / self._temperature, dim=1) * self._temperature
                    ...
                    if self._use_softplus:
                        diff1 = self.cql_gap_softplus_tau * F.softplus(
                            (raw_gap1 - self.cql_gap_margin) / self.cql_gap_softplus_tau
                        )
                        diff2 = self.cql_gap_softplus_tau * F.softplus(
                            (raw_gap2 - self.cql_gap_margin) / self.cql_gap_softplus_tau
                        )
                    else:
                        diff1 = F.relu(raw_gap1 - self.cql_gap_margin)
                        diff2 = F.relu(raw_gap2 - self.cql_gap_margin)

                    cql1_loss = diff1.mean()
                    cql2_loss = diff2.mean()
                    cql_gap = 0.5 * (cql1_loss + cql2_loss)
                    rand_q_mean = 0.5 * ((q1_rand - random_density).mean() + (q2_rand - random_density).mean())
                    curr_q_mean = 0.5 * ((q1_curr - curr_logp).mean() + (q2_curr - curr_logp).mean())
                    next_q_mean = 0.5 * ((q1_next - next_logp).mean() + (q2_next - next_logp).mean())
            ...
            return (
                ...
                curr_boundrate.detach(),
                next_boundrate.detach(),
            )
    ...
    def _ensure_online_rollout_state(self) -> None:
        if self._online_obs is not None and self._online_critic_obs is not None:
            return
        obs, critic_obs = self.env.reset_with_critic_obs()
        self._online_obs = torch.as_tensor(obs, device=self.device, dtype=torch.float32)
        self._online_critic_obs = torch.as_tensor(critic_obs, device=self.device, dtype=torch.float32)

    def _offline_ratio(self) -> float:
        if self.config.mixing_ratio_schedule == "fixed":
            ratio = self.config.offline_mixing_ratio
        elif self.config.mixing_ratio_schedule == "linear":
            anneal_steps = max(1, int(self.config.mixing_anneal_steps))
            progress = min(max(self.online_update_steps, 0) / anneal_steps, 1.0)
            ratio = self.config.offline_mixing_start + progress * (
                self.config.offline_mixing_end - self.config.offline_mixing_start
            )
        else:
            raise ValueError(f"Unknown mixing_ratio_schedule={self.config.mixing_ratio_schedule!r}")
        return min(max(float(ratio), 0.0), 1.0)

    def _policy_actions(self, obs: torch.Tensor, *, random_actions: bool) -> torch.Tensor:
        if random_actions:
            action_scale = self.actor.action_scale.to(device=self.device, dtype=torch.float32)
            action_bias = self.actor.action_bias.to(device=self.device, dtype=torch.float32)
            actions = torch.empty(
                obs.shape[0],
            ...
            return actions * action_scale + action_bias if self.config.use_tanh else actions
    ...
    def collect_online_steps(self, num_steps: int | None = None) -> int:
        ...
        self._ensure_online_rollout_state()
        assert self._online_obs is not None
        assert self._online_critic_obs is not None

        collected_steps = 0
        for _ in range(steps_to_collect):
            use_random_actions = (
                self.config.online_random_warmup
                and self.online_env_steps < self.config.online_warmup_steps
            )
            obs = self._online_obs
            critic_obs = self._online_critic_obs
            actions = self._policy_actions(obs, random_actions=use_random_actions)
            ...
            done_mask = dones.to(device=self.device, dtype=torch.bool)
            transition_next_obs = torch.where(
                done_mask[:, None],
                infos["observations"]["final"]["actor_obs"],
                next_obs,
            )
            transition_next_critic_obs = torch.where(
                done_mask[:, None],
            ...
            transition = TensorDict(
                {
                    ...
                    "next": {
                        "observations": transition_next_obs,
                        "critic_observations": transition_next_critic_obs,
                        "rewards": rewards.to(device=self.device, dtype=torch.float32),
                        "dones": dones.to(device=self.device, dtype=torch.long),
                        "truncations": truncations.to(device=self.device, dtype=torch.long),
                        "effective_n_steps": torch.ones(self.env.num_envs, device=self.device, dtype=torch.long),
                    },
                },
    ...
    def _sample_online_batch(self, batch_size: int, normalize_obs, normalize_critic_obs) -> TensorDict:
        batch = self.online_replay_buffer.sample(batch_size=batch_size)
        return self._prepare_online_batch(batch, normalize_obs, normalize_critic_obs)
    ...
    def _sample_mixed_batch(
        ...
        normalize_obs,
        normalize_critic_obs,
    ) -> tuple[TensorDict, int, int, float]:
        if len(self.online_replay_buffer) <= 0:
            data = self._sample_offline_batch(
                batch_size=batch_size,
                normalize_obs=normalize_obs,
                normalize_critic_obs=normalize_critic_obs,
            )
            return data, batch_size, 0, 1.0

        offline_ratio = self._offline_ratio()
        offline_batch_size = int(round(batch_size * offline_ratio))
        offline_batch_size = min(max(offline_batch_size, 0), batch_size)
        online_batch_size = batch_size - offline_batch_size
        if online_batch_size == 0 and batch_size > 1:
            online_batch_size = 1
            offline_batch_size = batch_size - 1

        batches: list[TensorDict] = []
        if offline_batch_size > 0:
            batches.append(
                self._sample_offline_batch(
    ...
    def online_learn(self, max_steps: int | None = None) -> None:
        ...
        if max_steps is None:
            max_steps = args.eval_interval if args.eval_interval > 0 else args.online_total_steps
        if max_steps <= 0:
            return
        if len(self.online_replay_buffer) <= 0:
            raise RuntimeError("Online replay buffer is empty. Run collect_online_steps() before online_learn().")

        target_step = self.global_step + int(max_steps)
        if args.compile:
        ...
        while self.global_step < target_step:
                    ...
                    self.training_metrics.add(
                        {
                            ...
                            "o2o/offline_mixing_ratio": torch.tensor(offline_ratio, device=self.device),
                            "o2o/offline_batch_size": torch.tensor(float(offline_bs), device=self.device),
                            "o2o/online_batch_size": torch.tensor(float(online_bs), device=self.device),
                            "o2o/online_env_steps": torch.tensor(float(self.online_env_steps), device=self.device),
                            "o2o/online_buffer_size": torch.tensor(float(len(self.online_replay_buffer)), device=self.device),
                            **action_ood_stats,
                        }
                    )
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving OS-CAL-QL model at global step {self.global_step}")
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if checkpoint_action_mode != "env_scaled_action_training_v1":
            logger.warning(
                "Loading a legacy checkpoint with different CAL-QL action semantics "
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required CAL-QL keys: {missing_required}. "
                "Expected a scalar-CAL-QL/CQL-compatible checkpoint."
            )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline CAL-QL gradient updates sample only the fixed dataset. "
        ...
        if self._offline_gpu_cache is not None:
            logger.info(
                f"Sampling CAL-QL offline dataset from GPU cache loaded from '{self._offline_dataset_path}' "
                f"with {self._offline_num_samples} samples."
            )
        else:
            logger.info(
                f"Streaming CAL-QL offline dataset from '{self._offline_dataset_path}' "
                f"with {self._offline_num_samples} samples."
            )
        while self.global_step < target_step:
            self.global_step += 1
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving CAL-QL model at global step {self.global_step}")
                    self.save(os.path.join(self.log_dir, f"model_{self.global_step:07d}.pt"))
                    self.export(onnx_file_path=os.path.join(self.log_dir, f"model_{self.global_step:07d}.onnx"))
    ...
    def evaluate_policy(self, max_eval_steps: int | None = None):
        try:
            ...
            self.reset_online_rollout_state()
