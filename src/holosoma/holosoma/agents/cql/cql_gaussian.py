...
class Actor(nn.Module):
    ...
    def _sample_random_mask(
        ...
        active_dim = max(0, min(active_dim, action_dim))
        if active_dim == action_dim:
            return torch.ones(batch_size, action_dim, device=device)
        if active_dim == 0:
            return torch.zeros(batch_size, action_dim, device=device)

        scores = torch.rand(batch_size, action_dim, device=device)
        active_idx = scores.topk(active_dim, dim=1).indices
        mask = torch.zeros(batch_size, action_dim, device=device)
