"""Pair-Synergy Body-Part Factorized CQL.

PBF-CQL targets pairwise synergy overestimation in pseudo-agent humanoid
decomposition.  BF-CQL/CFCQL singleton counterfactuals suppress
v_i = Q(s, a_pi^i, a_D^{-i}) - Q(s, a_D).  However, in a single humanoid
decomposed into body-part groups, overestimation can arise only when two body
parts drift simultaneously.  Therefore PBF-CQL computes all pair
counterfactuals and penalizes only the residual:

    Delta_ij = v_ij - v_i - v_j

not v_ij itself.  This avoids double-counting singleton conservatism and
reduces to BF-CQL when Q is group-additive.  This module is intentionally
standalone: it copies the BF-CQL training style but does not inherit from or
alias methods from the BF-CQL agent class.
"""
...
def build_group_to_action_mask(
    ...
    *,
    ...
    """Build a [G, A] boolean map from body-part groups to action dimensions."""

    group_to_action_mask = torch.zeros(len(group_indices), action_dim, dtype=torch.bool, device=device)
    for group_idx, indices in enumerate(group_indices):
        group_to_action_mask[group_idx, list(indices)] = True
    return group_to_action_mask


def build_all_pair_group_masks(
    num_groups: int,
    *,
    device: torch.device | str,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Build all i<j pair masks.

    Returns pair_masks [P, G], pair_i [P], and pair_j [P].  For the default G1
    functional grouping G=9, this produces P=C(9,2)=36 pairs.
    """
    ...
    pair_masks = torch.tensor(masks, dtype=torch.bool, device=device)
    pair_i_tensor = torch.tensor(pair_i, dtype=torch.long, device=device)
    pair_j_tensor = torch.tensor(pair_j, dtype=torch.long, device=device)
    if num_groups == 9:
        assert pair_masks.shape[0] == 36, f"Expected 36 PBF pairs for G=9, got {pair_masks.shape[0]}"
    return pair_masks, pair_i_tensor, pair_j_tensor


def build_singleton_group_masks(
    num_groups: int,
    *,
    device: torch.device | str,
) -> torch.Tensor:
    """Build singleton masks as an identity matrix with shape [G, G]."""

    return torch.eye(num_groups, dtype=torch.bool, device=device)
...
def counterfactual_actions_from_group_masks(
    ...
    """Construct counterfactual actions from group masks without batch loops.

    `group_masks` may be [N, G] or [B, N, G].  Output shape is always [B, N, A].
    Action dimensions covered by selected groups come from `actor_actions`;
    the remaining dimensions stay anchored to `dataset_actions`.
    """

    if group_masks.dim() == 2:
        action_masks = torch.matmul(
            group_masks.to(device=dataset_actions.device, dtype=torch.float32),
            group_to_action_mask.to(device=dataset_actions.device, dtype=torch.float32),
        ).to(torch.bool)
        action_masks = action_masks.unsqueeze(0)
    elif group_masks.dim() == 3:
        action_masks = torch.matmul(
            group_masks.to(device=dataset_actions.device, dtype=torch.float32),
            group_to_action_mask.to(device=dataset_actions.device, dtype=torch.float32),
        ).to(torch.bool)
    else:
        raise ValueError(f"group_masks must have rank 2 or 3, got shape={tuple(group_masks.shape)}")
...
class PBFCQLAgent(BaseAlgo):
    config: BFCQLConfig
    env: PBFCQLEnv  # type: ignore[assignment]
    ...
    def __init__(
        ...
        wrapped_env = PBFCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
    ...
    def setup(self) -> None:
        logger.info("Setting up pair-synergy body-part factorized offline CQL")
        ...
        if args.use_cnn_encoder:
            raise ValueError("PBF-CQL currently supports vector observations only; set use_cnn_encoder=False.")
        ...
        if not args.use_tanh:
            raise ValueError("PBF-CQL requires use_tanh=True because actor/critic training uses normalized actions.")
        ...
        logger.info("PBF-CQL action semantics: actor/critic always use normalized u-space [-1, 1].")
        ...
        self.pbf_group_to_action_mask = build_group_to_action_mask(
            list(group_indices),
            n_act,
            device=device,
        )
        self.pbf_singleton_masks = build_singleton_group_masks(len(group_indices), device=device)
        self.pbf_pair_masks, self.pbf_pair_i, self.pbf_pair_j = build_all_pair_group_masks(
            len(group_indices),
            device=device,
        )
        if args.ood_actor_num != 1:
            logger.warning(
                "PBF-CQL assumes singleton BF-CQL base term. "
                "Set ood_actor_num=1 for clean experiments."
            )
        logger.info(
            f"PBF-CQL all-pair counterfactuals: G={len(group_indices)}, "
            f"P={int(self.pbf_pair_masks.shape[0])}"
        )
        ...
        logger.info(f"PBF-CQL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        logger.info(
            "PBF-CQL action groups: "
            + ", ".join(
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized PBF-CQL model parameters across {self.gpu_world_size} GPUs")
    ...
    def _compute_pbf_pair_penalty(
        ...
        """Compute all-pair PBF-CQL synergy penalty.

        For each pair i<j:

            v_i  = Q(s, a_cf({i})) - Q(s, a_D)
            v_ij = Q(s, a_cf({i,j})) - Q(s, a_D)
            Delta_ij = v_ij - v_i - v_j

        The hinge is on Delta_ij rather than v_ij because singleton
        overestimation is already handled by the BF-CQL/CFCQL base term.  The
        singleton values and the data-action anchor are detached, so gradients
        from this loss flow only through pair counterfactual Q-values.
        """

        pbf_cfg = getattr(self.config, "pbf_cql", None)
        enabled = getattr(pbf_cfg, "enabled", True)
        alpha_pbf = float(getattr(pbf_cfg, "alpha", 0.05))
        margin = float(getattr(pbf_cfg, "margin", 0.0))
        use_softplus = bool(getattr(pbf_cfg, "use_softplus", False))
        beta = float(getattr(pbf_cfg, "softplus_beta", 1.0))
        num_pairs = int(self.pbf_pair_masks.shape[0])

        zero = torch.zeros((), device=self.device, dtype=q1_data.dtype)
        if not enabled or alpha_pbf <= 0.0:
            metrics = {
                "pbf/loss": zero.detach(),
                "pbf/penalty": zero.detach(),
                "pbf/delta_mean": zero.detach(),
                "pbf/delta_max": zero.detach(),
                "pbf/active_rate": zero.detach(),
                "pbf/v_pair_mean": zero.detach(),
                "pbf/v_single_sum_mean": zero.detach(),
                "pbf/num_pairs": torch.tensor(float(num_pairs), device=self.device, dtype=q1_data.dtype),
            }
            return zero, metrics
        if beta <= 0.0:
            raise ValueError(f"pbf_cql.softplus_beta must be > 0, got {beta}")

        batch_size = int(dataset_actions.shape[0])
        action_dim = int(dataset_actions.shape[-1])
        critic_obs_dim = int(critic_observations.shape[-1])
        num_groups = int(self.pbf_singleton_masks.shape[0])
        ...
        with torch.no_grad():
            ...
            singleton_actions = counterfactual_actions_from_group_masks(
                ...
                group_masks=self.pbf_singleton_masks,
                group_to_action_mask=self.pbf_group_to_action_mask,
            )
            singleton_critic_observations = (
                critic_observations[:, None, :]
                .expand(batch_size, num_groups, critic_obs_dim)
                .reshape(batch_size * num_groups, critic_obs_dim)
            )
            q1_single, q2_single = self.qnet(
                singleton_critic_observations,
                singleton_actions.reshape(batch_size * num_groups, action_dim),
            )
            q1_single = q1_single.view(batch_size, num_groups)
            q2_single = q2_single.view(batch_size, num_groups)

        pair_actions = counterfactual_actions_from_group_masks(
            dataset_actions=dataset_actions,
            actor_actions=actor_actions,
            group_masks=self.pbf_pair_masks,
            group_to_action_mask=self.pbf_group_to_action_mask,
        )
        pair_critic_observations = (
            critic_observations[:, None, :]
            .expand(batch_size, num_pairs, critic_obs_dim)
            .reshape(batch_size * num_pairs, critic_obs_dim)
        )
        q1_pair, q2_pair = self.qnet(
            pair_critic_observations,
            pair_actions.reshape(batch_size * num_pairs, action_dim),
        )
        q1_pair = q1_pair.view(batch_size, num_pairs)
        q2_pair = q2_pair.view(batch_size, num_pairs)

        q1_data_anchor = q1_data.detach()
        q2_data_anchor = q2_data.detach()

        v1_single = (q1_single - q1_data_anchor[:, None]).detach()
        v2_single = (q2_single - q2_data_anchor[:, None]).detach()
        v1_pair = q1_pair - q1_data_anchor[:, None]
        v2_pair = q2_pair - q2_data_anchor[:, None]

        single_sum1 = v1_single[:, self.pbf_pair_i] + v1_single[:, self.pbf_pair_j]
        single_sum2 = v2_single[:, self.pbf_pair_i] + v2_single[:, self.pbf_pair_j]
        delta1 = v1_pair - single_sum1.detach()
        delta2 = v2_pair - single_sum2.detach()

        if use_softplus:
            penalty1 = F.softplus((delta1 - margin) / beta) * beta
            penalty2 = F.softplus((delta2 - margin) / beta) * beta
        else:
            penalty1 = F.relu(delta1 - margin)
            penalty2 = F.relu(delta2 - margin)

        pbf_penalty = 0.5 * (penalty1.mean() + penalty2.mean())
        pbf_loss = alpha_pbf * pbf_penalty
        metrics = {
            "pbf/loss": pbf_loss.detach(),
            "pbf/penalty": pbf_penalty.detach(),
            "pbf/delta_mean": (0.5 * (delta1.mean() + delta2.mean())).detach(),
            "pbf/delta_max": (0.5 * (delta1.max() + delta2.max())).detach(),
            "pbf/active_rate": (
                0.5 * ((delta1 > margin).float().mean() + (delta2 > margin).float().mean())
            ).detach(),
            "pbf/v_pair_mean": (0.5 * (v1_pair.mean() + v2_pair.mean())).detach(),
            "pbf/v_single_sum_mean": (0.5 * (single_sum1.mean() + single_sum2.mean())).detach(),
            "pbf/num_pairs": torch.tensor(float(num_pairs), device=self.device, dtype=q1_data.dtype),
        }
        return pbf_loss, metrics
    ...
    def _update_q(
        ...
        with self._maybe_amp():
            ...
            pbf_loss, pbf_metrics = self._compute_pbf_pair_penalty(
            ...
            q_loss = bellman_loss + conservative_loss + pbf_loss
        ...
        return (
            ...
            pbf_metrics["pbf/loss"].detach(),
            pbf_metrics["pbf/penalty"].detach(),
            pbf_metrics["pbf/delta_mean"].detach(),
            pbf_metrics["pbf/delta_max"].detach(),
            pbf_metrics["pbf/active_rate"].detach(),
            pbf_metrics["pbf/v_pair_mean"].detach(),
            pbf_metrics["pbf/v_single_sum_mean"].detach(),
            pbf_metrics["pbf/num_pairs"].detach(),
        )
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if checkpoint_action_mode != "normalized_action_training_v1":
            logger.warning(
                "Loading a legacy checkpoint with different PBF-CQL action semantics "
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required PBF-CQL keys: {missing_required}. "
                "Expected a PBF-CQL-compatible checkpoint."
            )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline PBF-CQL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            with self.logging_helper.record_learn_time():
                for _ in range(args.num_updates):
                    ...
                    (
                        ...
                        pbf_active_rate,
                        pbf_v_pair_mean,
                        pbf_v_single_sum_mean,
                        pbf_num_pairs,
                    ) = update_q(data)
                    pbf_metrics = {
                        "pbf/loss": pbf_loss,
                        "pbf/penalty": pbf_penalty,
                        "pbf/delta_mean": pbf_delta_mean,
                        "pbf/delta_max": pbf_delta_max,
                        "pbf/active_rate": pbf_active_rate,
                        "pbf/v_pair_mean": pbf_v_pair_mean,
                        "pbf/v_single_sum_mean": pbf_v_single_sum_mean,
                        "pbf/num_pairs": pbf_num_pairs,
                    }
                    ...
                    self.training_metrics.add(
                        {
                            ...
                            "pbf_cql/ood_actor_num": torch.tensor(
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving PBF-CQL model at global step {self.global_step}")
    ...
    def save(self, path: str) -> None:  # type: ignore[override]
        ...
        metadata["algo"] = "pbf_cql"
        metadata["bf_cql_action_grouping"] = self.config.bf_cql_action_grouping
        pbf_cfg = getattr(self.config, "pbf_cql", None)
        metadata["pbf_cql"] = {
            "enabled": getattr(pbf_cfg, "enabled", True),
            "alpha": getattr(pbf_cfg, "alpha", 0.05),
            "margin": getattr(pbf_cfg, "margin", 0.0),
            "use_softplus": getattr(pbf_cfg, "use_softplus", False),
            "softplus_beta": getattr(pbf_cfg, "softplus_beta", 1.0),
            "num_pairs": int(getattr(self, "pbf_pair_masks").shape[0]),
        }
        save_params(
            self.global_step,
            self.actor,
