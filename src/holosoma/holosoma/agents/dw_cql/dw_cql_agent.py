"""Independent distributed-placement Advantage-Weighted CQL implementation.

This file intentionally owns the complete scalar CQL training loop instead of
inheriting from CQLAgent or AWCQLAgent. It uses the same fixed, globally
mean-normalized AW sidecar weight at exactly three placements: the complete
conservative bracket, the twin Bellman residual, and the complete SAC actor
objective. Bellman targets, entropy-alpha tuning, CQL Lagrange tuning, target
networks, twin aggregation, sampling proposals, and optimizers remain unchanged.
This is a placement ablation using this project's weight signal, not a
reproduction of another method's learned-weight construction.
"""
...
from holosoma.config_types.algo import DWCQLConfig
...
def transition_weighted_mean(
    per_sample_loss: torch.Tensor,
    weight: torch.Tensor,
) -> torch.Tensor:
    """Return ``mean(w_i * loss_i)`` without batch renormalization."""
    if per_sample_loss.shape != weight.shape:
        raise ValueError(
            f"DW-CQL weight/loss shape mismatch: weight={tuple(weight.shape)} "
            f"loss={tuple(per_sample_loss.shape)}"
        )
    return (weight.to(device=per_sample_loss.device, dtype=per_sample_loss.dtype) * per_sample_loss).mean()


def weighted_twin_bellman_loss(
    q1: torch.Tensor,
    q2: torch.Tensor,
    ...
    """Weight completed twin Bellman residuals while leaving ``q_target`` unchanged."""
    if loss_type == "huber":
        q1_loss = F.smooth_l1_loss(q1, q_target, beta=huber_beta, reduction="none")
        q2_loss = F.smooth_l1_loss(q2, q_target, beta=huber_beta, reduction="none")
    elif loss_type == "mse":
        q1_loss = (q1 - q_target).square()
        q2_loss = (q2 - q_target).square()
    else:
        raise ValueError(f"Unsupported DW-CQL Bellman loss type: {loss_type}")
    return transition_weighted_mean(q1_loss + q2_loss, weight)


def unweighted_entropy_alpha_loss(
    log_alpha: torch.Tensor,
    log_probs: torch.Tensor,
    target_entropy: float,
) -> torch.Tensor:
    """Track target entropy from the unweighted policy distribution.

    DW placement weights the complete actor objective, but never the automatic
    entropy-temperature controller. Mixing the transition sidecar into this
    dual update would make alpha track batch composition instead of the global
    policy entropy target.
    """
...
class DWCQLAgent(BaseAlgo):
    """Standalone CQL implementation with distributed sidecar-weight placement."""

    config: DWCQLConfig
    env: DWCQLEnv  # type: ignore[assignment]
    actor: Actor
    qnet: DoubleQCritic
    ...
    def __init__(
        ...
        wrapped_env = DWCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
        ...
        self._dw_weight_table: torch.Tensor | None = None
        self._dw_batch_weight: torch.Tensor | None = None
        self._dw_last_dataset_index: torch.Tensor | None = None
        self._dw_last_metrics: dict[str, torch.Tensor] = {}
    ...
    def _load_dw_weights(self) -> None:
        """Load and validate the fixed transition sidecar used by every DW placement."""
        sidecar_path = Path(self.config.aw_weights_path or f"{self._offline_dataset_path}.aw_weights.npz")
        logger.info(
            "DW-CQL pairing paths: h5='{}', sidecar='{}'",
        ...
        with np.load(sidecar_path, allow_pickle=False) as sidecar:
            if "n" not in sidecar or "weight" not in sidecar:
                raise KeyError(f"DW-CQL sidecar '{sidecar_path}' must contain 'n' and 'weight'.")
        ...
        if sidecar_size != self._offline_num_samples:
            raise ValueError(
                f"DW-CQL sidecar / H5 size mismatch: sidecar '{sidecar_path}' has n={sidecar_size}, "
        ...
        if weights.shape != (self._offline_num_samples,):
            raise ValueError(
                f"DW-CQL sidecar weight shape must be ({self._offline_num_samples},), got {weights.shape}."
            )
        if not np.isfinite(weights).all() or np.any(weights < 0.0):
            raise ValueError("DW-CQL sidecar weights must be finite and non-negative.")

        self._dw_weight_table = torch.as_tensor(weights, dtype=torch.float32, device=self.device)
        self._dw_last_metrics = {
            "dw_cql/batch_ess": torch.zeros((), device=self.device),
            "dw_cql/batch_w_mean": torch.zeros((), device=self.device),
            "dw_cql/batch_w_max": torch.zeros((), device=self.device),
        }
        logger.info(
            "DW-CQL weights loaded: n={} beta={:.6f} ESS/N={:.3f} clip%={:.2f} mean(w)={:.6f}",
            sidecar_size,
            beta,
            ess_fraction,
            100.0 * clip_fraction,
    ...
    def _install_dw_index_capture(self) -> None:
        """Capture sampled H5 row indices without changing sampler ordering."""
        sampler = self._offline_gpu_cache if self._offline_gpu_cache is not None else self._offline_shuffle_buffer
        if sampler is None:
            raise RuntimeError("DW-CQL requires an initialized offline sampler.")
        ...
        def sample_with_index(batch_size: int):
            ...
            if "dataset_index" not in batch:
                raise RuntimeError("Offline sampler batch has no 'dataset_index'; DW-CQL cannot map weights.")
            self._dw_last_dataset_index = batch["dataset_index"]
            return batch

        sampler.sample = sample_with_index  # type: ignore[method-assign]

    def _dw_weight_like(self, reference: torch.Tensor) -> torch.Tensor:
        """Return the current row-wise sidecar weight in a loss tensor's dtype/device."""
        if self._dw_batch_weight is None:
            raise RuntimeError("DW-CQL batch weights are unavailable; sample a dataset batch first.")
        weight = self._dw_batch_weight.to(device=reference.device, dtype=reference.dtype)
        if weight.shape != reference.shape:
            raise RuntimeError(
                f"DW-CQL weight/loss shape mismatch: weight={tuple(weight.shape)} loss={tuple(reference.shape)}"
            )
        return weight

    def setup(self) -> None:
        logger.info("Setting up independent offline DW-CQL")
        ...
        self._load_dw_weights()
        self._install_dw_index_capture()
        logger.info(
            "DW-CQL placement active: one fixed sidecar weights the CQL bracket, "
            "Bellman residual, and complete SAC actor objective; targets and alpha updates are unweighted."
        )
    ...
    def _compute_action_ood_stats(self, data: TensorDict) -> dict[str, torch.Tensor]:
        ...
        stats.update(self._dw_last_metrics)
        return stats
    ...
    def _update_q(
        ...
        with self._maybe_amp():
            ...
            bellman_weight = self._dw_weight_like(q1)
            # The target y is unchanged; w multiplies only the completed twin residual.
            bellman_loss = weighted_twin_bellman_loss(
                q1,
                q2,
                q_target,
                bellman_weight,
                loss_type=args.bellman_loss_type,
            ...
            if self._cql_weight > 0.0:
                ...
                conservative_weight = self._dw_weight_like(cql1_per_sample)
                # Weight the full LSE-minus-anchor bracket, never its sources separately.
                cql1_loss = transition_weighted_mean(cql1_per_sample, conservative_weight)
                cql2_loss = transition_weighted_mean(cql2_per_sample, conservative_weight)
        ...
        if self.config.use_autotune:
            ...
            with self._maybe_amp():
                ...
                # Deliberately unweighted: w affects the actor objective, not
                # the entropy-temperature controller or its target statistic.
                alpha_loss = unweighted_entropy_alpha_loss(
                    self.log_alpha,
                    log_probs,
                    self.target_entropy,
                )
    ...
    def _update_actor(
        ...
        with self._maybe_amp():
            ...
            actor_per_sample = self.log_alpha.exp().detach() * log_probs - qf_value
            actor_weight = self._dw_weight_like(actor_per_sample)
            actor_rl_loss = transition_weighted_mean(actor_per_sample, actor_weight)
    ...
    def _sample_offline_batch(
        ...
        if self._dw_weight_table is None or self._dw_last_dataset_index is None:
            raise RuntimeError("DW-CQL sidecar/index state was not initialized.")
        indices = self._dw_last_dataset_index.to(device=self.device, dtype=torch.long)
        weight = self._dw_weight_table[indices]
        if effective_batch_size != weight.shape[0]:
            if effective_batch_size % weight.shape[0] != 0:
                raise RuntimeError(
                    f"Effective batch size {effective_batch_size} is not a multiple of sampled "
                    f"batch size {weight.shape[0]}; cannot align DW weights."
                )
            weight = weight.repeat(effective_batch_size // weight.shape[0])
        self._dw_batch_weight = weight
        data["dw_weight"] = weight
        with torch.no_grad():
            batch_ess = weight.sum().square() / (weight.square().sum().clamp_min(1e-12) * weight.numel())
            self._dw_last_metrics = {
                "dw_cql/batch_ess": batch_ess,
                "dw_cql/batch_w_mean": weight.mean(),
                "dw_cql/batch_w_max": weight.max(),
            }
        return data
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline DW-CQL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving DW-CQL model at global step {self.global_step}")
