...
SYNC-QL is managed independently from BF-CQL (own package, no shared
classes); it uses the same factorized-actor design, global twin critic, replay loader,
...
from holosoma.agents.sync_cql.sync_cql import DoubleQCritic, FactorizedActor, resolve_action_groups
from holosoma.agents.sync_cql.sync_cql_utils import EmpiricalNormalization, save_params
...
class SyncCQLAgent(BaseAlgo):
    ...
    config: BFCQLConfig
    env: SyncCQLEnv  # type: ignore[assignment]
    ...
    def __init__(
        ...
        wrapped_env = SyncCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
    ...
    def setup(self) -> None:
        logger.info("Setting up SYNC-QL (drift-gated CFCQL)")
        ...
        if not args.use_tanh:
            raise ValueError("BF-CQL requires use_tanh=True because actor/critic training uses normalized actions.")
        ...
        logger.info("BF-CQL action semantics: actor/critic always use normalized u-space [-1, 1].")
