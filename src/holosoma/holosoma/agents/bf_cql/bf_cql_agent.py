...
class BFCQLAgent(BaseAlgo):
    ...
    def setup(self) -> None:
        logger.info("Setting up body-part factorized offline CQL")
        ...
        if not args.use_tanh:
            raise ValueError("BF-CQL requires use_tanh=True for bounded action training.")
        ...
        if self.normalized_action_training:
            ...
            logger.info("BF-CQL action semantics: actor/critic use normalized u-space [-1, 1].")
        else:
            action_scale = env_action_scale
            action_bias = env_action_bias
            self.action_space_mode = "env_scaled_action_training_v1"
            logger.info("BF-CQL action semantics: actor/critic use legacy env-scaled action space.")
        ...
        logger.info(f"BF-CQL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        logger.info(
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized BF-CQL model parameters across {self.gpu_world_size} GPUs")
        ...
        with self._maybe_amp():
            ...
            if self._cql_weight > 0.0:
                ...
                for group_idx, group_indices in enumerate(self.bf_cql_group_indices):
                    ...
                    group_index_tensor = torch.as_tensor(group_indices, device=self.device, dtype=torch.long)
                    group_action_scale = self.actor.action_scale.to(
                        device=self.device,
                        dtype=dataset_actions.dtype,
                    ).index_select(0, group_index_tensor)
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if compatible_checkpoint_action_mode != expected_action_mode:
            logger.warning(
                "Loading a legacy checkpoint with different BF-CQL action semantics "
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required BF-CQL keys: {missing_required}. "
                "Expected a BF-CQL checkpoint."
            )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline BF-CQL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving BF-CQL model at global step {self.global_step}")
