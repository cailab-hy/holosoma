"""DCQL: escape-ray Directional Conservative Q-Learning.

DCQL is a standalone offline SAC/CQL-style agent that keeps the BF-CQL runtime
plumbing: factorized actor heads, a global twin critic, normalized action-space
training, target networks, replay loading, observation normalization, and export.
The conservative term is not BF-CQL/SYNC-CQL.  For each state it measures the
direction from an on-support reference action ``a_ref`` (dataset action in v1) to
the deterministic actor action ``a_pi``.  If the sigma-normalized drift is large,
DCQL evaluates Q on escape-ray actions ``a_ref + t * (a_pi - a_ref) + eps`` and
penalizes their log-sum-exp against the anchor ``Q(s, a_D)``.

Only critic parameters receive gradients from the ray loss.  Ray actions and the
per-sample gate are built under ``torch.no_grad()``; actor training remains the
...
from holosoma.agents.dcql.dcql import DoubleQCritic, FactorizedActor, resolve_action_groups
from holosoma.agents.dcql.dcql_utils import EmpiricalNormalization, save_params
...
class DCQLAgent(BaseAlgo):
    """Standalone DCQL agent using BF-CQL-compatible modules without class inheritance."""

    config: BFCQLConfig
    env: DCQLEnv  # type: ignore[assignment]
    ...
    def __init__(
        ...
        wrapped_env = DCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
        ...
        self._lagrange_warning_logged = False
    ...
    def setup(self) -> None:
        logger.info("Setting up DCQL (escape-ray directional CQL)")
        ...
        if args.use_cnn_encoder:
            raise ValueError("DCQL currently supports vector observations only; set use_cnn_encoder=False.")
        ...
        if not args.use_tanh:
            raise ValueError("DCQL requires use_tanh=True because actor/critic training uses normalized actions.")
        ...
        logger.info("DCQL action semantics: actor/critic always use normalized u-space [-1, 1].")
        ...
        logger.info(f"DCQL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        logger.info(
            "DCQL factorized actor groups: "
            + ", ".join(
    ...
    def _setup_dcql(self) -> None:
        dcql = self.config.dcql
        action_dim = int(self.env.robot_config.actions_dim)

        if dcql.ray_noise_std < 0.0:
            raise ValueError(f"dcql.ray_noise_std must be >= 0, got {dcql.ray_noise_std}")
        if dcql.delta_thr < 0.0:
            raise ValueError(f"dcql.delta_thr must be >= 0, got {dcql.delta_thr}")
        if dcql.gate_norm not in {"batch", "active"}:
            raise ValueError(f"dcql.gate_norm must be 'batch' or 'active', got {dcql.gate_norm!r}")
        if dcql.a_ref_mode not in {"dataset", "knn"}:
            raise ValueError(f"dcql.a_ref_mode must be 'dataset' or 'knn', got {dcql.a_ref_mode!r}")
        if dcql.a_ref_mode == "knn":
            raise NotImplementedError(
                "dcql.a_ref_mode='knn' requires a behavior/reference-action index. "
                "Plug in the KNN reference provider before enabling this mode."
            )
        if dcql.drift_std_momentum < 0.0 or dcql.drift_std_momentum >= 1.0:
            raise ValueError(
                f"dcql.drift_std_momentum must be in [0, 1), got {dcql.drift_std_momentum}"
            )
        if dcql.warmup_ballast_steps < 0:
            raise ValueError(f"dcql.warmup_ballast_steps must be >= 0, got {dcql.warmup_ballast_steps}")
        if dcql.ballast_alpha < 0.0:
            raise ValueError(f"dcql.ballast_alpha must be >= 0, got {dcql.ballast_alpha}")
        if dcql.ballast_num_samples < 0:
            raise ValueError(f"dcql.ballast_num_samples must be >= 0, got {dcql.ballast_num_samples}")
        if len(dcql.t_grid) == 0:
            raise ValueError("dcql.t_grid must contain at least one ray coefficient")
        if any(t <= 0.0 for t in dcql.t_grid):
            raise ValueError(f"dcql.t_grid values must be > 0, got {dcql.t_grid}")
        if self.config.use_lagrange and not self._lagrange_warning_logged:
            logger.warning(
                "DCQL currently uses fixed cql_weight for the escape-ray conservative term. "
                "use_lagrange=True is accepted for config compatibility but is not applied to DCQL."
            )
            self._lagrange_warning_logged = True

        self.dcql_t_grid = torch.tensor(dcql.t_grid, device=self.device, dtype=torch.float32)
        self.sync_action_std = torch.ones(action_dim, device=self.device)

        logger.info(
            "DCQL escape-ray regularization enabled: "
            f"enabled={dcql.enabled}, t_grid={list(dcql.t_grid)}, ray_noise_std={dcql.ray_noise_std}, "
            f"delta_thr={dcql.delta_thr}, gate_norm={dcql.gate_norm}, a_ref_mode={dcql.a_ref_mode}"
        )

    def _dcql_disabled(self) -> bool:
        return (not self.config.dcql.enabled) or self._cql_weight <= 0.0

    @torch.no_grad()
    def _update_dcql_action_std(self, dataset_actions: torch.Tensor) -> None:
        dcql = self.config.dcql
        if dcql.freeze_drift_stats:
            return
        batch_std = dataset_actions.detach().float().std(dim=0, unbiased=False).clamp_min(1e-3)
        self.sync_action_std.mul_(dcql.drift_std_momentum).add_(batch_std, alpha=1.0 - dcql.drift_std_momentum)

    @torch.no_grad()
    def _get_dcql_reference_actions(
        self,
        observations: torch.Tensor,
        dataset_actions: torch.Tensor,
    ) -> torch.Tensor:
        del observations
        if self.config.dcql.a_ref_mode == "dataset":
            return dataset_actions.detach()
        if self.config.dcql.a_ref_mode == "knn":
            raise NotImplementedError(
                "dcql.a_ref_mode='knn' requires a behavior/reference-action index. "
                "Plug in the KNN reference provider before enabling this mode."
            )
        raise ValueError(f"Unknown dcql.a_ref_mode={self.config.dcql.a_ref_mode!r}")

    @torch.no_grad()
    def _build_dcql_rays(
        self,
        a_ref: torch.Tensor,
        a_pi: torch.Tensor,
    ) -> torch.Tensor:
        """Build escape-ray actions a_ref + t * (a_pi - a_ref) + eps in normalized action space."""

        dcql = self.config.dcql
        t_grid = self.dcql_t_grid.to(device=a_ref.device, dtype=a_ref.dtype)
        delta = a_pi.detach() - a_ref.detach()
        rays = a_ref[:, None, :] + t_grid[None, :, None] * delta[:, None, :]
        if dcql.ray_noise_std > 0.0:
            rays = rays + torch.randn_like(rays) * dcql.ray_noise_std
        return rays.clamp(-1.0, 1.0)

    def _update_q(
        self,
        data: TensorDict,
        use_ballast: bool = False,
    ):
        args = self.config
        dcql = args.dcql
        scaler = self.scaler
        ...
        with self._maybe_amp():
            ...
            with torch.no_grad():
                ...
                next_q_mean = next_target_min_q.mean()
            ...
            zero = torch.zeros((), device=self.device, dtype=bellman_loss.dtype)
            conservative_loss = zero
            cql_gap = zero
            rand_q_mean = zero
            curr_logp_mean = zero
            next_logp_mean = zero
            random_density_mean = zero
            dcql_delta_mean = zero
            dcql_delta_p50 = zero
            dcql_delta_p90 = zero
            dcql_delta_p95 = zero
            dcql_active_frac = zero
            dcql_gap_gated = zero
            dcql_gap_active = zero
            dcql_ballast_loss = zero
            num_rays = int(self.dcql_t_grid.numel()) if hasattr(self, "dcql_t_grid") else 1
            dcql_q_ray_by_t = torch.zeros(num_rays, device=self.device, dtype=bellman_loss.dtype)
            dcql_active_mask = torch.zeros(dataset_actions.shape[0], device=self.device, dtype=torch.bool)

            with torch.no_grad():
                # FactorizedActor.forward returns tanh(mean) when use_tanh=True, so this is
                # the deterministic squashed actor action in normalized critic action space.
                pi_actions_det = self.actor(observations)[0]
                q1_pi_det, q2_pi_det = self.qnet(critic_observations, pi_actions_det)
                q_pi_det = torch.minimum(q1_pi_det, q2_pi_det)
                q_pi_minus_q_data = (q_pi_det - q_data.detach()).mean()
                curr_q_mean = q_pi_det.mean()

                if not self._dcql_disabled():
                    self._update_dcql_action_std(dataset_actions)
                    a_ref = self._get_dcql_reference_actions(observations, dataset_actions)
                    delta = pi_actions_det.detach() - a_ref.detach()
                    normalized_delta = delta / (
                        self.sync_action_std.to(device=dataset_actions.device, dtype=dataset_actions.dtype) + 1e-6
                    )
                    delta_norm = torch.sqrt(normalized_delta.pow(2).mean(dim=1) + 1e-12)
                    dcql_active_mask = delta_norm >= float(dcql.delta_thr)
                    dcql_delta = delta_norm.to(dtype=bellman_loss.dtype)
                    dcql_delta_mean = dcql_delta.mean().detach()
                    dcql_delta_p50 = torch.quantile(dcql_delta.float(), 0.50).to(dtype=bellman_loss.dtype).detach()
                    dcql_delta_p90 = torch.quantile(dcql_delta.float(), 0.90).to(dtype=bellman_loss.dtype).detach()
                    dcql_delta_p95 = torch.quantile(dcql_delta.float(), 0.95).to(dtype=bellman_loss.dtype).detach()
                    dcql_active_frac = dcql_active_mask.to(dtype=bellman_loss.dtype).mean().detach()
                    ray_actions = self._build_dcql_rays(a_ref, pi_actions_det)
                else:
                    ray_actions = torch.zeros(
            ...
            if not self._dcql_disabled():
                ...
                num_rays = ray_actions.shape[1]
                ray_critic_obs = critic_observations[:, None, :].expand(batch_size, num_rays, -1).reshape(
                    batch_size * num_rays,
                    -1,
                )
                ray_actions_flat = ray_actions.reshape(batch_size * num_rays, -1)
                q1_ray, q2_ray = self.qnet(ray_critic_obs, ray_actions_flat)
                q1_ray = q1_ray.view(batch_size, num_rays)
                q2_ray = q2_ray.view(batch_size, num_rays)
                ray_min_q = torch.minimum(q1_ray, q2_ray)
                dcql_q_ray_by_t = ray_min_q.mean(dim=0).detach()

                log_num_rays = math.log(float(num_rays))
                q1_ray_lse = self._temperature * (
                    torch.logsumexp(q1_ray / self._temperature, dim=1) - log_num_rays
                )
                ...
                cql1_loss = (gap1 * gate_float).sum() / gate_denominator
                cql2_loss = (gap2 * gate_float).sum() / gate_denominator
                cql_gap = 0.5 * (cql1_loss + cql2_loss)
                conservative_loss = self._cql_weight * cql_gap
                dcql_gap_gated = cql_gap.detach()
                active_denominator = gate_float.sum().clamp_min(1.0)
                dcql_gap_active = (0.5 * (gap1 + gap2) * gate_float).sum() / active_denominator
                rand_q_mean = ray_min_q.mean().detach()

                if use_ballast and dcql.ballast_alpha > 0.0 and dcql.ballast_num_samples > 0:
                    num_ballast = int(dcql.ballast_num_samples)
                    ballast_actions = torch.empty(
                        batch_size * num_ballast,
                        dataset_actions.shape[-1],
                        device=self.device,
                        dtype=dataset_actions.dtype,
                    ).uniform_(-1.0, 1.0)
                    ballast_critic_obs = critic_observations[:, None, :].expand(batch_size, num_ballast, -1).reshape(
                        batch_size * num_ballast,
                        -1,
                    )
                    q1_ballast, q2_ballast = self.qnet(ballast_critic_obs, ballast_actions)
                    q1_ballast = q1_ballast.view(batch_size, num_ballast)
                    q2_ballast = q2_ballast.view(batch_size, num_ballast)
                    log_num_ballast = math.log(float(num_ballast))
                    ballast_gap1 = self._temperature * (
                        torch.logsumexp(q1_ballast / self._temperature, dim=1) - log_num_ballast
                    ) - q1
                    ballast_gap2 = self._temperature * (
                        torch.logsumexp(q2_ballast / self._temperature, dim=1) - log_num_ballast
                    ) - q2
                    dcql_ballast_loss = 0.5 * (ballast_gap1.mean() + ballast_gap2.mean())
                    conservative_loss = conservative_loss + self._cql_weight * dcql.ballast_alpha * dcql_ballast_loss
        ...
        return (
            ...
            dcql_delta_mean.detach(),
            dcql_delta_p50.detach(),
            dcql_delta_p90.detach(),
            dcql_delta_p95.detach(),
            dcql_active_frac.detach(),
            dcql_gap_gated.detach(),
            dcql_gap_active.detach(),
            dcql_q_ray_by_t.detach(),
            dcql_ballast_loss.detach(),
            dcql_active_mask.detach(),
        )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline DCQL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            with self.logging_helper.record_learn_time():
                for _ in range(args.num_updates):
                    ...
                    use_ballast = self._critic_update_step < args.dcql.warmup_ballast_steps
                    (
                        reward_mean,
                        q_grad_norm,
                        q_loss,
                    ...
                    dcql_metric_dict = {
                        "dcql/delta_norm_mean": dcql_delta_mean,
                        "dcql/delta_norm_p50": dcql_delta_p50,
                        "dcql/delta_norm_p90": dcql_delta_p90,
                        "dcql/delta_norm_p95": dcql_delta_p95,
                        "dcql/active_frac": dcql_active_frac,
                        "dcql/gap_gated": dcql_gap_gated,
                        "dcql/gap_active": dcql_gap_active,
                        "dcql/penalty": conservative_loss,
                        "dcql/q_data_mean": q_data_mean,
                        "dcql/ballast_loss": dcql_ballast_loss,
                        "dcql/use_ballast": torch.tensor(float(use_ballast), device=self.device),
                    }
                    for ray_idx, t_value in enumerate(args.dcql.t_grid):
                        if ray_idx >= int(dcql_q_ray_by_t.shape[0]):
                            break
                        t_label = str(float(t_value)).replace(".", "p")
                        dcql_metric_dict[f"dcql/q_ray_t{t_label}"] = dcql_q_ray_by_t[ray_idx]
                    ...
                    if "motion_phase" in data:
                        ...
                        if bool(valid_phase.any().item()):
                            sample_active = dcql_active_mask.to(dtype=torch.float32)
                            for bin_idx in range(5):
                                ...
                                dcql_metric_dict[f"dcql/phase_bin_{bin_idx}/active_frac"] = (
                    ...
                    self.training_metrics.add(
                        {
                            ...
                            "cql_alpha_value": torch.tensor(0.0, device=self.device),
                            "cql_lagrange_loss": torch.tensor(0.0, device=self.device),
                            "cql_target_action_gap": torch.tensor(0.0, device=self.device),
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving DCQL model at global step {self.global_step}")
    ...
    def save(self, path: str) -> None:  # type: ignore[override]
        ...
        metadata["dcql"] = {
            "enabled": self.config.dcql.enabled,
            "t_grid": list(self.config.dcql.t_grid),
            "ray_noise_std": self.config.dcql.ray_noise_std,
            "delta_thr": self.config.dcql.delta_thr,
            "gate_norm": self.config.dcql.gate_norm,
            "a_ref_mode": self.config.dcql.a_ref_mode,
            "drift_std_momentum": self.config.dcql.drift_std_momentum,
            "freeze_drift_stats": self.config.dcql.freeze_drift_stats,
            "warmup_ballast_steps": self.config.dcql.warmup_ballast_steps,
            "ballast_alpha": self.config.dcql.ballast_alpha,
            "ballast_num_samples": self.config.dcql.ballast_num_samples,
        }
        metadata["sync_action_std"] = self.sync_action_std.detach().cpu()
        save_params(
            self.global_step,
            self.actor,
            self.qnet,
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if checkpoint_action_mode != "normalized_action_training_v1":
            logger.warning(
                "Loading a legacy checkpoint with different DCQL action semantics "
                f"(action_space_mode={checkpoint_action_mode})."
            )
        checkpoint_algo = torch_checkpoint.get("algo")
        if checkpoint_algo is not None and checkpoint_algo != "dcql":
            logger.warning(f"Loading checkpoint tagged algo={checkpoint_algo!r} into DCQLAgent.")
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required DCQL keys: {missing_required}. "
                "Expected a DCQL-compatible checkpoint."
            )
        ...
        checkpoint_action_std = torch_checkpoint.get("sync_action_std")
        if isinstance(checkpoint_action_std, torch.Tensor):
            if int(checkpoint_action_std.numel()) != int(self.env.robot_config.actions_dim):
                raise RuntimeError(
                    "Checkpoint/config mismatch for action dimension: "
                    f"checkpoint sync_action_std has {int(checkpoint_action_std.numel())} dims, "
                    f"current env has {int(self.env.robot_config.actions_dim)} dims."
                )
            self.sync_action_std.copy_(checkpoint_action_std.to(device=self.device, dtype=self.sync_action_std.dtype))
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized DCQL model parameters across {self.gpu_world_size} GPUs")
