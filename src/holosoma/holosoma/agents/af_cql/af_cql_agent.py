...
from holosoma.agents.af_cql.af_cql import DoubleQCritic, FactorizedActor, resolve_action_groups
from holosoma.agents.af_cql.af_cql_utils import EmpiricalNormalization, save_params
from holosoma.agents.af_cql.syndiag import (
    build_coalitions,
...
class AFCQLAgent(BaseAlgo):
    config: BFCQLConfig
    env: AFCQLEnv  # type: ignore[assignment]
    ...
    def __init__(
        ...
        wrapped_env = AFCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
        ...
        if config.af_cql.bcpa_lambda < 0.0:
            raise ValueError(f"af_cql.bcpa_lambda must be >= 0, got {config.af_cql.bcpa_lambda}")
        if config.af_cql.num_active_groups <= 0:
            raise ValueError(
                f"af_cql.num_active_groups must be > 0, got {config.af_cql.num_active_groups}"
            )
    ...
    def setup(self) -> None:
        logger.info("Setting up AF-CQL: plain-CQL critic + BCPA (block-coordinate) actor")
        ...
        if args.use_cnn_encoder:
            raise ValueError("AF-CQL currently supports vector observations only; set use_cnn_encoder=False.")
        ...
        if not args.use_tanh:
            raise ValueError("AF-CQL requires use_tanh=True for bounded action training.")
        ...
        if self.normalized_action_training:
            ...
            logger.info("AF-CQL action semantics: actor/critic use normalized u-space [-1, 1].")
        else:
            action_scale = env_action_scale
            action_bias = env_action_bias
            self.action_space_mode = "env_scaled_action_training_v1"
            logger.info("AF-CQL action semantics: actor/critic use legacy env-scaled action space.")
        ...
        # BCPA block structure: same physical action groups as BF-CQL.
        af_group_dim_mask = torch.zeros(len(group_indices), n_act, device=device, dtype=torch.float32)
        for af_group_idx, af_dims in enumerate(group_indices):
            af_group_dim_mask[af_group_idx, list(af_dims)] = 1.0
        self._af_group_dim_mask = af_group_dim_mask
        ...
        logger.info(f"AF-CQL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        logger.info(
            "AF-CQL action groups: "
            + ", ".join(
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized AF-CQL model parameters across {self.gpu_world_size} GPUs")
    ...
    def _update_q(
        ...
        with self._maybe_amp():
            ...
            if self._cql_weight > 0.0:
                ...
                # Plain CQL(H) over the FULL action vector (identical to the standalone
                # CQL agent): importance-sampled logsumexp over uniform-random,
                # current-policy and next-policy proposals. No group factorization.
                with torch.no_grad():
                ...
                counterfactual_actions = torch.cat([rand_actions, curr_actions, next_actions_rep], dim=0)
                ...
                cql1_loss = (torch.logsumexp(q1_terms / self._temperature, dim=1) * self._temperature - q1).mean()
                cql2_loss = (torch.logsumexp(q2_terms / self._temperature, dim=1) * self._temperature - q2).mean()
                ...
                curr_logp_mean = curr_logp.mean()
                next_logp_mean = next_logp.mean()
                random_density_mean = random_density.to(dtype=bellman_loss.dtype)
    ...
    def _sample_bcpa_action_mask(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor]:
        """Sample the BCPA block mask m over action dimensions.

        Blocks are the BF-CQL physical action groups. By default one mask is
        drawn per training step (af_cql.mask_per_sample=False); optionally an
        independent block is drawn per batch sample.

        Returns (action_mask [rows, A] float, group_mask [rows, G] bool) with
        rows = batch_size when mask_per_sample else 1.
        """
        af = self.config.af_cql
        num_groups = len(self.bf_cql_group_indices)
        num_active = min(max(int(af.num_active_groups), 1), num_groups)
        rows = batch_size if af.mask_per_sample else 1
        scores = torch.rand(rows, num_groups, device=self.device)
        active_group_ids = scores.topk(num_active, dim=1).indices
        group_mask = torch.zeros(rows, num_groups, device=self.device, dtype=torch.bool)
        group_mask.scatter_(1, active_group_ids, True)
        action_mask = (group_mask.to(self._af_group_dim_mask.dtype) @ self._af_group_dim_mask).clamp_(max=1.0)
        return action_mask, group_mask
    ...
    def _update_actor(
        ...
        """BCPA actor update: ascend Q along a sampled block of action dims only.

            L_A = alpha * log pi(a|s) - lambda * Q(s, m * a_pi + (1 - m) * a_D)

        a_pi is the reparameterized actor action (grad flows), a_D the dataset
        action (no grad), m a block mask over action dims sampled per training
        step from the physical action groups. Critic losses are NOT modified.
        """
        args = self.config
        af = args.af_cql
        scaler = self.scaler
        ...
        with self._maybe_amp():
            ...
            dataset_actions = self._to_critic_actions(data["actions"]).detach()  # [B, act_dim]
            ...
            action_mask, _ = self._sample_bcpa_action_mask(actions.shape[0])
            action_mask = action_mask.to(dtype=actions.dtype)
            mixed_actions = action_mask * actions + (1.0 - action_mask) * dataset_actions

            q1_pi, q2_pi = self.qnet(critic_observations, mixed_actions)
            qf_value = torch.minimum(q1_pi, q2_pi)
            actor_rl_loss = (self.log_alpha.exp().detach() * log_probs - af.bcpa_lambda * qf_value).mean()
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if compatible_checkpoint_action_mode != expected_action_mode:
            logger.warning(
                "Loading a legacy checkpoint with different AF-CQL action semantics "
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required AF-CQL keys: {missing_required}. "
                "Expected a AF-CQL checkpoint."
            )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline AF-CQL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving AF-CQL model at global step {self.global_step}")
