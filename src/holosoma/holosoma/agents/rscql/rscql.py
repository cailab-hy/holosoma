...
class FactorizedActor(Actor):
    ...
    def get_actions_and_log_prob_per_dim(self, obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Sampled action plus PER-DIMENSION log-probs.

        RSC-QL sums these over per-step random blocks, so the density bookkeeping
        stays exact for arbitrary partitions (not just the fixed head grouping).
        RNG consumption is identical to get_actions_and_group_log_probs.
        """
        _, mean, log_std = self(obs)
        std = log_std.exp()
        dist = torch.distributions.Normal(mean, std)
