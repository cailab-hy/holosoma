...
from holosoma.agents.rscql.rscql import DoubleQCritic, FactorizedActor, resolve_action_groups
from holosoma.agents.rscql.rscql_utils import EmpiricalNormalization, save_params
from holosoma.agents.rscql.syndiag import (
    build_coalitions,
...
class RSCQLAgent(BaseAlgo):
    config: BFCQLConfig
    env: RSCQLEnv  # type: ignore[assignment]
    ...
    def __init__(
        ...
        wrapped_env = RSCQLEnv(env, config.actor_obs_keys, config.critic_obs_keys)
        ...
        self._rsc_partition: list[torch.Tensor] | None = None
        if config.rscql.resample_interval <= 0:
            raise ValueError(
                f"rscql.resample_interval must be > 0, got {config.rscql.resample_interval}"
            )
    ...
    def setup(self) -> None:
        logger.info("Setting up RSC-QL: CQL with per-step random-subspace conservatism")
        ...
        if args.use_cnn_encoder:
            raise ValueError("RSC-QL currently supports vector observations only; set use_cnn_encoder=False.")
        ...
        if not args.use_tanh:
            raise ValueError("RSC-QL requires use_tanh=True for bounded action training.")
        ...
        if self.normalized_action_training:
            ...
            logger.info("RSC-QL action semantics: actor/critic use normalized u-space [-1, 1].")
        else:
            action_scale = env_action_scale
            action_bias = env_action_bias
            self.action_space_mode = "env_scaled_action_training_v1"
            logger.info("RSC-QL action semantics: actor/critic use legacy env-scaled action space.")
        ...
        # RSC: random-partition block sizes mirror the physical preset, so the
        # identity permutation reproduces the fixed BF-CQL partition exactly.
        # (Actor heads stay physically grouped; only the counterfactual
        # partition is re-drawn.)
        self._rsc_block_sizes = [len(g) for g in group_indices]
        self._rsc_n_act = n_act
        logger.info(
            f"RSC-QL random-subspace blocks: sizes={self._rsc_block_sizes}, "
            f"resample_interval={args.rscql.resample_interval} critic updates"
        )
        ...
        logger.info(f"RSC-QL dims: actor_obs_dim={actor_obs_dim}, critic_obs_dim={critic_obs_dim}, n_act={n_act}")
        logger.info(
            "RSC-QL action groups: "
            + ", ".join(
    ...
    def _synchronize_model_parameters(self) -> None:
        ...
        self.qnet_target.load_state_dict(self.qnet.state_dict())
        logger.info(f"Synchronized RSC-QL model parameters across {self.gpu_world_size} GPUs")
    ...
    @torch.no_grad()
    def _sample_rsc_partition(self) -> list[torch.Tensor]:
        """Draw a fresh random partition of the action dims into fixed-size blocks.

        Called OUTSIDE the compiled critic step (offline_learn) and passed into
        _update_q as index tensors: shapes/count are static across steps, only the
        contents change, so torch.compile never retraces.
        """
        perm = torch.randperm(self._rsc_n_act, device=self.device)
        return list(torch.split(perm, self._rsc_block_sizes))

    def _rsc_counterfactual_actor_actions(
        self,
        base_actions: torch.Tensor,
        actor_actions: torch.Tensor,
        selected_group_mask: torch.Tensor,
        rsc_partition: list[torch.Tensor],
    ) -> torch.Tensor:
        """Blockwise actor counterfactuals over the random partition.

        Mirrors _counterfactual_actor_group_actions, but the block -> dim mapping
        comes from the per-step partition instead of the fixed physical groups.
        """
        counterfactual_actions = base_actions.clone()
        for group_idx, block_dims in enumerate(rsc_partition):
            row_ids = torch.nonzero(selected_group_mask[:, group_idx], as_tuple=False).flatten()
            if row_ids.numel() == 0:
                continue
            counterfactual_actions[row_ids[:, None], block_dims[None, :]] = actor_actions[
                row_ids[:, None], block_dims[None, :]
            ]
        return counterfactual_actions
    ...
    def _update_q(
        ...
        with self._maybe_amp():
            ...
            if self._cql_weight > 0.0:
                ...
                # RSC: identical CQL(H) structure to BF-CQL, but the blocks come from
                # the per-step random partition. Densities are exact for arbitrary
                # blocks via per-dimension log-probs summed over each block.
                with torch.no_grad():
                    ...
                    curr_group_logps_stacked = torch.stack(
                        [curr_logp_per_dim.index_select(1, block_dims).sum(dim=-1) for block_dims in rsc_partition],
                        dim=1,
                    )
                    next_group_logps_stacked = torch.stack(
                        [next_logp_per_dim.index_select(1, block_dims).sum(dim=-1) for block_dims in rsc_partition],
                        dim=1,
                    )
                ...
                num_groups_int = len(rsc_partition)
                for group_idx, block_dims in enumerate(rsc_partition):
                    group_dim = self._rsc_block_sizes[group_idx]
                    group_action_scale = self.actor.action_scale.to(
                        device=self.device,
                        dtype=dataset_actions.dtype,
                    ).index_select(0, block_dims)
                    group_action_bias = self.actor.action_bias.to(
                        device=self.device,
                        dtype=dataset_actions.dtype,
                    ).index_select(0, block_dims)
                    ...
                    rand_group_actions = rand_group_actions * group_action_scale + group_action_bias
                    random_density = (
                        torch.tensor(math.log(0.5) * group_dim, device=self.device, dtype=dataset_actions.dtype)
                        - torch.log(group_action_scale + 1e-6).sum()
                    )

                    rand_counterfactual_actions = expanded_dataset_actions.clone()
                    rand_counterfactual_actions[:, block_dims] = rand_group_actions
                    ...
                    curr_counterfactual_actions = self._rsc_counterfactual_actor_actions(
                        expanded_dataset_actions,
                        curr_actions,
                        selected_group_mask,
                        rsc_partition,
                    )
                    next_counterfactual_actions = self._rsc_counterfactual_actor_actions(
                        expanded_dataset_actions,
                        next_actions_rep,
                        selected_group_mask,
                        rsc_partition,
                ...
                num_groups = float(len(rsc_partition))
    ...
    def load(self, ckpt_path: str | None) -> None:
        ...
        if compatible_checkpoint_action_mode != expected_action_mode:
            logger.warning(
                "Loading a legacy checkpoint with different RSC-QL action semantics "
        ...
        if missing_required:
            raise RuntimeError(
                f"Checkpoint missing required RSC-QL keys: {missing_required}. "
                "Expected a RSC-QL checkpoint."
            )
    ...
    def offline_learn(self, max_steps: int | None = None) -> None:
        ...
        if self.env.num_envs > 1 and self.is_main_process:
            logger.info(
                "Offline RSC-QL gradient updates sample only the fixed dataset. "
        ...
        while self.global_step < target_step:
            ...
            with self.logging_helper.record_learn_time():
                for _ in range(args.num_updates):
                    ...
                    # RSC: re-draw the random block partition OUTSIDE the compiled
                    # graph; fixed shapes keep torch.compile from retracing.
                    if self._rsc_partition is None or (
                        self._critic_update_step % max(int(args.rscql.resample_interval), 1) == 0
                    ):
                        self._rsc_partition = self._sample_rsc_partition()
                    (
                        reward_mean,
                        q_grad_norm,
                        q_loss,
                        ...
                        syndiag_q_data_min,
                        syndiag_pi_actions,
                    ) = update_q(data, self._rsc_partition)
            ...
            if args.save_interval > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    logger.info(f"Saving RSC-QL model at global step {self.global_step}")
