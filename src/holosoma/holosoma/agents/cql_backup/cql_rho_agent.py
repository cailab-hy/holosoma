...
class CQL_RHO_Agent(BaseAlgo):
    ...
    def __init__(
        ...
        # CQL(rho): tail-fraction used to aggregate conservative gaps.
        # We reuse existing config field `cvar_alpha` as rho in (0, 1].
        self._rho = float(config.cvar_alpha)
    ...
    def setup(self) -> None:
        logger.info(f"Setting up scalar offline CQL(rho) with rho={self._rho:.3f}")
    ...
    def _rho_tail_mean(self, values: torch.Tensor) -> torch.Tensor:
        ...
        if values.ndim != 1:
            raise ValueError(f"Expected 1D tensor for rho aggregation, got shape {tuple(values.shape)}")
        if self._rho >= 1.0:
            return values.mean()

        k = max(1, int(math.ceil(values.numel() * self._rho)))
    ...
    def _update_q(
        ...
        with self._maybe_amp():
            ...
            rand_actions = torch.empty(bsz * num_repeat, dataset_actions_u.shape[-1], device=self.device).uniform_(-1.0, 1.0)
            ...
            cql1_loss = self._rho_tail_mean(cql1_per_sample)
            cql2_loss = self._rho_tail_mean(cql2_per_sample)
